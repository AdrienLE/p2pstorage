...
public:
    ...
    virtual JellyInternalStatus::type prepareAddPart(std::string const & id, int64_t size, ClientProof const & client, int64_t total_size);
    virtual JellyInternalStatus::type addPart(std::string const &salt, std::string const & id, std::string const & file, ClientProof const & client, int64_t total_size);
